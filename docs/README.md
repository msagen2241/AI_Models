...
- Validator: checks that the plan still matches reality and highlights risky assumptions before execution.
- Builder: creates the folders, files, and code required by the validated plan with small, reversible changes.
- Reviewer: audits the resulting changes for maintainability, safety, and path or import fallout.

## Command workflow
1. Start OpenCode in this workspace.
2. Tell the planner what you want to build and the explicit folder path where it should live.
3. Run `oc run --command plan-structure` to write `.opencode/plans/code-structure-plan.md`, including the target project folder path and the exact implementation plan.
4. Run `oc run --command validate-structure` to verify the plan against the current filesystem state and confirm it is ready to execute.
5. Run `oc run --command implement-structure` to create the target project folder when needed and then create or modify the folders, files, and code described by the validated plan.
6. Run `oc run --command review-structure` to inspect the implementation and capture residual risk.

## New project workflow
- Example request: `Create a MUD project with a teleport ability in G:\MidTier\AI-Projects\MudTeleport`.
- If no path is provided for a new project, the planner should ask for one before planning.
- The builder should create the target folder first, then build inside it.
...
If future work needs MCP, add named integrations directly under the `mcp` object in `opencode.json` without making them mandatory for local coding tasks.
